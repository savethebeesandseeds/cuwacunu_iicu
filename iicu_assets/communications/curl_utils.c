...
void init_curl_string(curl_string_t *s){
    ...
    if(s->ptr == NULL){
        fprintf(stderr,"on init_curl_string; malloc() failed to allocate memory\n");
        exit(EXIT_FAILURE);
    }
    s->ptr[0]='\0';
}
size_t load_curl_string(void *ptr, size_t size, size_t nmemb, curl_string_t *s){
    ...
    if(s->ptr == NULL){
        fprintf(stderr,"on load_curl_string; realloc() failed to allocate memory\n");
        exit(EXIT_FAILURE);
    }
...
curl_string_t get_request(char *target_url){
    ...
    if(curl) {
        ...
        s.res_code = curl_easy_perform(curl); // failing (#FIXME)
        /* Check for errors */
