...
void sdl_draw_main_plot(__iicu_wikimyei_t *_iicu_wikimyei){
    ...
    latest_alliu_relative_y=(int)((_kemu->alliu_latest-main_box_min)*((__cwcn_type_t)_nijcyota->main_box_h)/(main_box_max-main_box_min+1e-9));
    sprintf(aux_caption,"%.8f",_kemu->alliu_latest);
    ...
    latest_alliu_relative_y=(int)((main_box_max-_kemu->alliu_latest)*((__cwcn_type_t)_nijcyota->main_box_h)/(main_box_max-main_box_min+1e-9));
    if(latest_alliu_relative_y>SCREEN_FONT_SIZE){
