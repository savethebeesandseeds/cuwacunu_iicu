...
void render_state_panel(sdl_screen_object_t *obj_sdl){
    ...
    icon_x=x_zero;
    icon_y=y_zero;
    draw_png(icon_path,icon_x,icon_y,icon_w,icon_h,obj_sdl->renderer);
...
void negate_icon(sdl_screen_object_t *obj_sdl,int x, int y, int w, int h){
    ...
    SDL_RenderDrawLine(obj_sdl->renderer, x, y, x+w, y+h);
    SDL_RenderDrawLine(obj_sdl->renderer, x, 1+y, x+w, y+h+1);
    SDL_RenderDrawLine(obj_sdl->renderer, x, 2+y, x+w, y+h+2);
}