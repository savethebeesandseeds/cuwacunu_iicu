...
__intelligent_order_policy_t *policy_fabric(){
    __intelligent_order_policy_t *new_iicu_policy=malloc(sizeof(__intelligent_order_policy_t));
...
__intelligent_order_policy_t *policy_clone_fabric(__intelligent_order_policy_t *src_iicu_policy){
    __intelligent_order_policy_t *new_iicu_policy=policy_fabric();
...
void run_intelligent_order_policy_on_mewaajacunwe(__iicu_jkimyei_t *_iicu_jkimyei, int _start_index){ // #FIXME change to mewaajacune_list
    // __intelligent_order_policy_t *c_policy=_iicu_jkimyei->__jk_inteligent_order_policy;
    ...
    __intelligent_order_policy_t *c_policy=_iicu_jkimyei->__jk_inteligent_order_policy;
    int index_a=0x00,index_b=_start_index-0x01;
    int l_size=_iicu_jkimyei->__jk_mewaajacune->__load_size;
    __cwcn_type_t size_flip = 0x01;
    __cwcn_type_t alliu_a,alliu_b,alliu_delta,comission_delta;
    while(index_b+0x01<l_size){
        index_a=index_b+0x01; // if jump
        alliu_a=c_mewaajcune_list[index_a];
        for(index_b=index_a+0x01;index_b<l_size && index_b<l_size;index_b++){
            alliu_b=c_mewaajcune_list[index_b];
            if(alliu_b>=(0x01+c_policy->__order_margin_uper)*alliu_a){
                alliu_delta=size_flip*(alliu_b-alliu_a)/alliu_a;
                comission_delta=(2+alliu_delta)*BROKER_MARKET_TOTAL_COMMISSION;
                c_policy->__policy_total_reward+=(alliu_delta-comission_delta)*c_policy->__base_margin_investment;
                size_flip = -0x01;
                break;
            } else if(alliu_b<=(0x01-c_policy->__order_margin_down)*alliu_a){
                alliu_delta=size_flip*(alliu_b-alliu_a)/alliu_a;
...
void monte_carlo_random_search(__iicu_jkimyei_t *_iicu_jkimyei){
    fprintf(stdout,"[%s cuwacunu:jkimyei %s] %s [ start ] %s monte_carlo_random_search\n",COLOR_CUWACUNU,COLOR_REGULAR,COLOR_JKIMYEI,COLOR_REGULAR);
    // --- --- --- 
    __intelligent_order_policy_t *best_policy=policy_fabric();
    ...
    // --- --- --- 
    __cwcn_type_t cum_reward=0x00;
    // --- --- --- 
    int rand_delta_mewaajacune=0x00;
    int mc_res=100;
    // --- --- --- 
    for(int ctx_episode=0x00;ctx_episode<MONTECARLO_EPISODES;ctx_episode++){
        ...
        cum_reward=0x00;
        // --- --- 
        for(int c_rep=0x00;c_rep<MONTECARLO_RAND_REPS;c_rep++){
            // --- 
            _iicu_jkimyei->__jk_inteligent_order_policy->__policy_total_reward=0x00;
            // --- 
            rand_delta_mewaajacune = ((int) max_min_res_rand(
                MONTECARLO_RAND_REPS_MAX, 
                MONTECARLO_RAND_REPS_LOW, 
                MONTECARLO_RAND_REPS));
            // --- 
            run_intelligent_order_policy_on_mewaajacunwe(_iicu_jkimyei, rand_delta_mewaajacune);
            // --- 
            cum_reward+=_iicu_jkimyei->__jk_inteligent_order_policy->__policy_total_reward;
            // --- 
        }
        // --- --- 
        _iicu_jkimyei->__jk_inteligent_order_policy->__policy_total_reward=((__cwcn_type_t)cum_reward)/((__cwcn_type_t)MONTECARLO_RAND_REPS);
        // --- --- 
...
void *jkimyei_launcher(void *_jk_thread_order){
    ...
    fprintf(stdout,"[%s cuwacunu:jkimyei %s] %s [ start ] %s jkimyei_launcher for type order : scene:[%d], kline:[%d]\n",COLOR_CUWACUNU,COLOR_REGULAR,COLOR_JKIMYEI,COLOR_REGULAR,c_scene_id,c_kline_id);
    ...
    // --- --- --- 
    fprintf(stdout,"[%s cuwacunu:jkimyei %s] %s [ printing policy ] %s : scene:[%d] kline:[%d]\n",COLOR_CUWACUNU,COLOR_REGULAR,COLOR_JKIMYEI,COLOR_REGULAR,c_scene_id,c_kline_id);
    ...
    get_jkimyei(temp_iicu_wikimyei, c_scene_id)->__jk_mewaajacune=get_mewaajacune(temp_iicu_wikimyei, c_scene_id, c_kline_id);
    // --- --- --- 
    release_mewaajacune(temp_iicu_wikimyei, c_scene_id, c_kline_id);
    release_jkimyei(temp_iicu_wikimyei, c_scene_id);
    // --- --- --- 
    destroy_mewaajacune(temp_iicu_jkimyei->__jk_mewaajacune);
    // --- --- --- 
    destroy_jkimyei(temp_iicu_jkimyei);
    // --- --- --- 
    ((__jkimyei_thread_order_t*)_jk_thread_order)->__jk_thead_is_bussy=0x00;
    fprintf(stdout,"[%s cuwacunu:jkimyei %s] %s [ end ] %s jkimyei_launcher : for type order : [scene:%d, kline:%d]\n",COLOR_CUWACUNU,COLOR_REGULAR,COLOR_JKIMYEI,COLOR_REGULAR,c_scene_id,c_kline_id);
    pthread_exit(NULL);
}
